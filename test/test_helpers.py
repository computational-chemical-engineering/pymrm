...
def test_construct_coefficient_matrix_flat_diagonal():
...
def test_construct_coefficient_matrix_single_shape_1d():
    """Single shape tuple: broadcasts coefficients to shape and makes diagonal."""
    n = 5
    shape = (n,)
    coeffs = np.ones(n) * 2.0
    mat = construct_coefficient_matrix(coeffs, shape=shape)
    assert mat.shape == (n, n)
    assert issparse(mat)
    np.testing.assert_allclose(mat.diagonal(), 2.0 * np.ones(n), atol=1e-14)


def test_construct_coefficient_matrix_single_shape_with_axis():
...
def test_construct_coefficient_matrix_broadcasts_scalar():
...
def test_construct_coefficient_matrix_pair_of_shapes():
...
def test_construct_coefficient_matrix_pair_of_shapes_with_axis():
    """Pair of tuples with axis: staggered face–cell coupling."""
    shape_rows = (1, 4)   # boundary face shape
    shape_cols = (5, 4)   # cell shape
    coeffs = np.ones((1, 4))
    mat = construct_coefficient_matrix(coeffs, shape=(shape_rows, shape_cols), axis=0)
...
def test_construct_coefficient_matrix_rectangular():
