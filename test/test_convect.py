...
def test_convflux_upwind_shapes():
    n = 8
    shape = (n,)
...
def test_convflux_upwind_negative_velocity():
...
def test_convflux_upwind_zero_velocity():
    """Zero velocity should result in zero flux."""
    n = 6
    shape = (n,)
    x_f = np.linspace(0, 1, n + 1)
    conv_int = construct_convflux_upwind_int(shape, v=0.0)
    c = np.ones(n)
    face_fluxes = conv_int @ c
    np.testing.assert_allclose(face_fluxes, 0.0, atol=1e-14)


def test_convflux_upwind_negative_velocity_with_bc():
...
def test_construct_convflux_bc_single_cell():
...
def test_construct_convflux_bc_single_cell_negative_v():
...
def test_construct_convflux_with_shapes_d():
...
def test_convflux_upwind_2d_shape():
...
def test_construct_convflux_bc_scalar_v():
    ...
    conv_matrix, conv_bc = construct_convflux_bc(shape, x_f, x_c=x_c, bc=bc, v=v_scalar)
    assert issparse(conv_matrix)
    assert issparse(conv_bc)


def test_construct_convflux_bc_scalar_v_negative():
    ...
    x_c = 0.5 * (x_f[:-1] + x_f[1:])
    bc = ({"a": 0, "b": 1, "d": 0.0}, {"a": 0, "b": 1, "d": 1.0})
    conv_matrix, conv_bc = construct_convflux_bc(shape, x_f, x_c=x_c, bc=bc, v=-1.0)
    assert issparse(conv_matrix)


def test_construct_convflux_upwind_int_scalar():
    """construct_convflux_upwind_int with scalar v (covers scalar path in int)."""
    n = 6
    shape = (n,)
    # v as integer scalar
    conv_matrix = construct_convflux_upwind_int(shape, v=2)
    assert issparse(conv_matrix)


def test_construct_convflux_upwind_int_scalar_shape():
    """construct_convflux_upwind with integer shape."""
    x_f = np.linspace(0, 1, 9)
    conv_matrix, conv_bc = construct_convflux_upwind(8, x_f)
