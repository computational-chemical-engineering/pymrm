...
def test_grad_and_div_shapes_1d():
    n = 8
...
def test_grad_int_shape():
    n = 6
    shape, x_f, x_c = _make_1d(n)
    grad_int = construct_grad_int(shape, x_f, x_c)
    assert grad_int.shape == (n + 1, n)


def test_construct_grad_accepts_int_shape():
    """construct_grad should accept a plain int for shape."""
    x_f = np.linspace(0, 1, 6)
    grad, _ = construct_grad(5, x_f)
...
def test_grad_linear_profile_internal():
...
def test_div_of_constant_flux_is_zero():
...
def test_laplacian_linear_profile():
...
def test_construct_grad_bc_single_cell():
...
def test_construct_convflux_single_cell_operators():
...
def test_grad_div_2d_shape():
    ...
    grad_x, _ = construct_grad(shape, x_f, axis=0)
    grad_y, _ = construct_grad(shape, y_f, axis=1)
...
def test_construct_div_callable_nu():
    """Custom callable geometry should work."""
    n = 8
    shape = (n,)
    x_f = np.linspace(0.0, 1.0, n + 1)
    div = construct_div(shape, x_f, nu=lambda r: np.ones_like(r))
    assert issparse(div)
    assert div.shape == (n, n + 1)
...
def test_construct_grad_with_shapes_d():
...
def test_construct_grad_bc_shapes_d_one_side_none():
...
def test_construct_grad_bc_shapes_d_single_cell():
